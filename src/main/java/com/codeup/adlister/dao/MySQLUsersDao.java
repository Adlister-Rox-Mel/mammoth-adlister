...
public class MySQLUsersDao extends MySQLDao implements Users {
    ...
    public Long insert(User user) {
        String query = "INSERT INTO users(username, email, password, phone, url) VALUES (?, ?, ?, ?, ?)";
        try {
    ...
    public void update(User user) {
        try {
            String updateQuery = "UPDATE users SET username = ?, email = ?, phone =?, url =? WHERE id = ?";
            PreparedStatement stmt = connection.prepareStatement(updateQuery, Statement.RETURN_GENERATED_KEYS);
            stmt.setString(1, user.getUsername());
            stmt.setString(2, user.getEmail());
            stmt.setString(3, user.getPhone());
            stmt.setString(4, user.getUrl());
            stmt.setFloat(5, user.getId());
            stmt.executeUpdate();
        } catch (SQLException e) {
